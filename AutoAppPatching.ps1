...
Function Test-RunningProcs{
    param(
        [String]$Program
    )
    # We want to assume a program is always running to err on the side of caution as to not interrupt a user
    # and attempt an update while they could be using the program.
    $appRunning = $true
    # Most apps listed by Sonicwall begin with the company name which will make filtering for 
    # the correct app easier.
    $progList = $Program.Split(" ")
    # However, some programs are only returned with one name (ex. Zoom)
    if($progList.Count -eq 1){
        $appName = $progList[0]
        $proc = Get-Process | Where-Object{$_.MainWindowTitle-like "*$appName*"}
    }else{
        $company = $progList[0]
        $appName = $progList[1..($progList.Count-1)]
        $proc = Get-Process | Where-Object{($_.Company -like "*$company*") -and ($_.MainWindowTitle-like "*$appName*")}
    }
    if($null -eq $proc){
        $appRunning = $false
    }
    # Possibly add some code here to handle expections where background apps can be stopped for updates.
    Return $appRunning
}
...
Function Update-Apps{
    ...
    foreach($app in $CVE_AppList){
        $tempAppList = $app.Split("-").Trim()
        Write-Host $tempAppList[0]
        $progOn = Test-RunningProcs -Program $tempAppList[0]
        if($progOn){
            # Function to create scheduled task to update app at next boot time
        }else{
